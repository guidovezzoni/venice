# Analytics Taxonomy Specification

## Purpose

Defines the complete event taxonomy and user-property vocabulary for the Venice analytics layer.
Governs the shape of every `AnalyticsEvent` subclass, every banding enum, the error classifier,
and the `AnalyticsUserProperty` sealed class — the elements that collectively form the
tracking-plan contract in code. This capability does not cover where tracking calls are placed
(see the `analytics-instrumentation` capability) or how the analytics layer is structured
(see the `analytics-abstraction` capability).
...
respectively. A mapping function SHALL classify an integer trip count into the correct band;
negative inputs SHALL be treated as zero.

#### Scenario: A negative count bands to zero
