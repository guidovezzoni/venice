...
Defines where and how tracking calls are placed in the Venice codebase. Governs which ViewModel
methods emit which events, when user properties are set, how failure paths are instrumented, and
how use-case return types support correct event parameterisation — without specifying the shape of
the events themselves (see the `analytics-taxonomy` capability) or the analytics layer's structure
