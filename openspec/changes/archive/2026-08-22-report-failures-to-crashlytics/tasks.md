...
- [x] 1.1 In `gradle/libs.versions.toml`, add a `firebaseCrashlyticsPlugin = "3.0.7"` version entry, a
...
- [x] 1.2 In root `build.gradle.kts`, add `alias(libs.plugins.firebase.crashlytics) apply false` to the
      `plugins {}` block, alongside the existing `alias(libs.plugins.google.services) apply false`.
- [x] 1.3 In `app/build.gradle.kts`, add `alias(libs.plugins.firebase.crashlytics)` to the `plugins {}`
...
- [x] 1.4 In `app/src/main/java/com/guidovezzoni/venice/di/FirebaseModule.kt`, add a
...
- [x] 2.2 Implement: create `app/src/main/java/com/guidovezzoni/venice/core/analytics/CrashlyticsAnalyticsProvider.kt`
...
- [x] 3.1 Write test: `GIVEN a CrashlyticsAnalyticsProvider with a mocked FirebaseCrashlytics WHEN logEvent(AnalyticsEvent.OperationFailed(AnalyticsOperation.CALCULATE_ROUTE, AnalyticsErrorType.NETWORK)) is called THEN setCustomKey("error_type", "network") is invoked` in `CrashlyticsAnalyticsProviderTest`.
- [x] 3.2 Implement: implement `logEvent(event)` with a `when (event)` branch matching only
...
- [x] 3.3 Write test: `GIVEN a CrashlyticsAnalyticsProvider with a mocked FirebaseCrashlytics WHEN logEvent(AnalyticsEvent.OperationFailed(AnalyticsOperation.CALCULATE_ROUTE, AnalyticsErrorType.NETWORK)) is called THEN recordException is never invoked on the mocked FirebaseCrashlytics` in `CrashlyticsAnalyticsProviderTest`.
- [x] 3.4 Implement: verify (no production code change expected — this test guards the invariant that
...
- [x] 3.5 Write test: `GIVEN a CrashlyticsAnalyticsProvider with a mocked FirebaseCrashlytics WHEN logEvent is called with an AnalyticsEvent that is not OperationFailed (e.g. TripCreated) THEN no method is invoked on the mocked FirebaseCrashlytics` in `CrashlyticsAnalyticsProviderTest`.
- [x] 3.6 Implement: ensure the `when (event)` in `logEvent` has an `else -> Unit` (or equivalent)
...
- [x] 4.1 Write test: `GIVEN AnalyticsUserProperty.TripCountBand(CountBand.RANGE_2_5) WHEN setUserProperty is called THEN FirebaseCrashlytics.setCustomKey is invoked with "trip_count_band" and "2_5"` in `CrashlyticsAnalyticsProviderTest`.
- [x] 4.2 Implement: implement `setUserProperty(property)` with an exhaustive `when (property)` over
...
- [x] 4.3 Write test: `GIVEN AnalyticsUserProperty.DistanceUnit(DistanceUnitParam.IMPERIAL) WHEN setUserProperty is called THEN FirebaseCrashlytics.setCustomKey is invoked with "distance_unit" and "imperial"` in `CrashlyticsAnalyticsProviderTest`.
- [x] 4.4 Implement: add the `DistanceUnit` branch to the same `when (property)`, calling
...
- [x] 6.1 In `app/src/main/java/com/guidovezzoni/venice/di/AnalyticsModule.kt`, add a
...
- [x] 6.2 Verify `app/proguard-rules.pro` needs no changes: confirm
      ...
      app).
- [x] 6.3 Search the codebase for `import com.google.firebase.crashlytics` outside
      `core/analytics/` and `di/` and confirm zero matches (per the spec's containment requirement).

## 7. Final verification

- [x] 7.1 Run `./gradlew assembleDebug` and `./gradlew assembleRelease` and confirm both build variants
      succeed with the Crashlytics plugin applied.
- [x] 7.2 Run `./gradlew testDebugUnitTest` and confirm all tests pass, including the new
      `CrashlyticsAnalyticsProviderTest`.
- [x] 7.3 Run `./gradlew detektDebug` and confirm zero findings.
- [x] 7.4 Run `./gradlew koverVerify` (or the project's configured Kover verification task) and confirm
      `CrashlyticsAnalyticsProvider` is covered at 95%+.
- [x] 7.5 Cross-check `docs/analytics/tracking-plan.md`: confirm no changes are needed — `trackException`
