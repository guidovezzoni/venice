...
- [ ] 1.4 In `LegSummary.kt`, define `private val IMPERIAL_UNIT_COUNTRY_CODES = setOf("US", "GB", "LR", "MM")` and `private const val METRES_PER_MILE = 1609.344`
...
- [ ] 2.9 Implement: convert `formatDistance` in `LegSummary.kt` from a private `@Composable` function to an `internal` plain function `formatDistance(distanceMetres: Int, locale: Locale, resources: Resources): String` that branches on `locale.country in IMPERIAL_UNIT_COUNTRY_CODES` (miles via `trip_detail_leg_distance_miles`) vs. the existing metric branch (metres/kilometres), resolving strings via `resources.getString(...)` instead of `stringResource()`
- [ ] 2.10 Implement: update the `LegSummary` composable to call `formatDistance(leg.distanceMetres, Locale.getDefault(), LocalContext.current.resources)`
...
- [ ] 3.3 Implement: confirm `LegSummary` composable requires no behavioural change beyond the `formatDistance` call-site update from task 2.10 (no clickable modifiers added); adjust only if test 3.2 reveals an issue
