...
- [ ] 1.1 Create `core/analytics/AnalyticsScreen.kt`: enum with `TRIP_LIST("trip_list")`,
  `TRIP_DETAIL("trip_detail")`
- [ ] 1.2 Create `core/analytics/StopTypeParam.kt`: enum with `STARTING_POINT("starting_point")`,
  `DESTINATION("destination")`, `INTERMEDIATE("intermediate")`
- [ ] 1.3 Create `core/analytics/AnalyticsErrorType.kt`: enum with `NETWORK("network")`,
...
- [ ] 1.4 Extend `core/analytics/AnalyticsOperation.kt`: add `UNDO_MARK_DEPARTED("undo_mark_departed")`,
  `SEARCH_PLACE("search_place")`, `RESOLVE_PLACE("resolve_place")` to the existing 7 values

## 2. Distance Banding (BDD)

- [ ] 2.1 Write test: GIVEN a distance of 0m, 49,999m, 50,000m, 199,999m, 200,000m, 999,999m,
...
- [ ] 2.2 Implement: `core/analytics/DistanceBand.kt` — enum with 5 constants each carrying
...
- [ ] 3.1 Write test: GIVEN a duration of 0s, 3,599s, 3,600s, 10,799s, 10,800s, 21,599s, 21,600s,
  ...
  `DurationBandTest`
- [ ] 3.2 Implement: `core/analytics/DurationBand.kt` — enum with 5 constants each carrying
  `value: String`, plus a mapping function classifying seconds into a band per design decision 1

## 4. Trip Count Banding (BDD)

- [ ] 4.1 Write test: GIVEN a trip count of 0, 1, 2, 5, and 6 WHEN the mapping function classifies
  each THEN it returns `ZERO`, `ONE`, `RANGE_2_5`, `RANGE_2_5`, `SIX_PLUS` respectively, in
  `CountBandTest`
- [ ] 4.2 Implement: `core/analytics/CountBand.kt` — enum with 4 constants each carrying
  `value: String`, plus a mapping function classifying a non-negative count into a band

## 5. Error Classification (BDD)

- [ ] 5.1 Write test: GIVEN a `SocketTimeoutException`, a plain `IOException`, a
...
- [ ] 5.2 Implement: `core/analytics/AnalyticsErrorClassifier.kt` — top-level function
...
- [ ] 6.1 Write test: GIVEN each of the 14 `AnalyticsEvent` subtypes (`TripCreated`, `TripOpened`,
...
- [ ] 6.2 Implement: rewrite `core/analytics/AnalyticsEvent.kt` — replace all 10 legacy subtypes with
...
- [ ] 7.1 Write test: GIVEN `AnalyticsUserProperty.TripCountBand(band = CountBand.RANGE_2_5)` and
...
- [ ] 7.2 Implement: rewrite `core/analytics/AnalyticsUserProperty.kt` — remove the `ForTesting`
  placeholder, add `TripCountBand(band: CountBand)` and `DistanceUnit(unit: String)`

## 8. Debug Log Formatter Regression Guard (BDD)

- [ ] 8.1 Write test: GIVEN one of the new `AnalyticsEvent` subtypes and one of the new
...
- [ ] 8.2 Implement: confirm `AnalyticsLogFormatter.kt` needs no code change (it operates generically
...
- [ ] 9.1 Write test: GIVEN `directionsApiService.fetchRoute` succeeds and legs are persisted WHEN
...
- [ ] 9.2 Implement: change `RouteRepository.calculateRoute` and `RouteRepositoryImpl.calculateRoute`
...
- [ ] 10.1 Write test: GIVEN the current pending stop at `order = 1` WHEN
...
- [ ] 10.2 Implement: change `MarkStopDepartedUseCase.invoke` return type to `Result<Stop>`; on
...
- [ ] 11.1 Write test: GIVEN the most recently departed stop at `order = 1` WHEN
...
- [ ] 11.2 Implement: change `UndoMarkStopDepartedUseCase.invoke` return type to `Result<Stop>`; on
...
- [ ] 12.1 Write test: GIVEN `TripListViewModel` is constructed THEN `ScreenViewed(AnalyticsScreen
...
- [ ] 12.2 Implement: update `TripListViewModel.init` to log `ScreenViewed(AnalyticsScreen
...
- [ ] 13.1 Write test: GIVEN the user had 1 trip before creating a new one WHEN `createTrip()`
...
- [ ] 13.2 Implement: update `createTrip()`'s success branch to compute `isFirstTrip` from the
...
- [ ] 14.1 Write test: GIVEN `createTripUseCase` fails with an `IOException` WHEN the failure branch
...
- [ ] 14.2 Implement: update `createTrip()`'s failure branch to classify the error via
...
- [ ] 15.1 Write test: GIVEN `onIntent(OnTripClicked(tripId))` is processed WHEN analytics calls are
  inspected THEN no `TripOpened` event is logged by `TripListViewModel`, in `TripListViewModelTest.kt`
- [ ] 15.2 Implement: remove the `analyticsClient.logEvent(AnalyticsEvent.TripOpened(...))` call from
  `onIntent(OnTripClicked)`

## 16. TripDetailViewModel — Screen View and Trip Opened on Init (BDD)

- [ ] 16.1 Write test: GIVEN `TripDetailViewModel` is constructed THEN `ScreenViewed(AnalyticsScreen
...
- [ ] 16.2 Implement: update `TripDetailViewModel.init` to log `ScreenViewed(AnalyticsScreen
...
- [ ] 17.1 Write test: GIVEN `uiState` reflects 2 stops before a starting point is set WHEN
...
- [ ] 17.2 Implement: update `setStop()`'s success branch to log `StopAdded(stopType.toParam(),
...
- [ ] 18.1 Write test: GIVEN `setStopUseCase` fails with a persistence error WHEN the failure branch
...
- [ ] 18.2 Implement: update `setStop()`'s failure branch to classify via `AnalyticsErrorClassifier`
  and call both `logEvent(OperationFailed(...))` and `trackException(...)`

## 19. TripDetailViewModel — Stop Edited (BDD)

- [ ] 19.1 Write test: GIVEN an intermediate stop is edited WHEN `editStop(...)` succeeds THEN
  `StopEdited(stopType = StopTypeParam.INTERMEDIATE)` is logged, in `TripDetailViewModelTest.kt`
- [ ] 19.2 Implement: update `editStop()`'s success branch to log `StopEdited(stopType.toParam())` —
  sourcing the edited stop's type from the stop being edited

## 20. TripDetailViewModel — Stop Edited Failure (BDD)

- [ ] 20.1 Write test: GIVEN `editStopUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 20.2 Implement: update `editStop()`'s failure branch for dual-channel emission

## 21. TripDetailViewModel — Stop Removed (BDD)

- [ ] 21.1 Write test: GIVEN `uiState` reflects 3 stops including the one being removed WHEN
  `removeStop()` succeeds THEN `StopRemoved(stopType, stopCount = 2)` is logged, in
  `TripDetailViewModelTest.kt`
- [ ] 21.2 Implement: update `removeStop()`'s success branch to log `StopRemoved(stopType.toParam(),
  currentStopCount() - 1)` per design decision 3

## 22. TripDetailViewModel — Stop Removed Failure (BDD)

- [ ] 22.1 Write test: GIVEN `removeStopUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 22.2 Implement: update `removeStop()`'s failure branch for dual-channel emission

## 23. TripDetailViewModel — Stop Reordered (BDD)

- [ ] 23.1 Write test: GIVEN `moveStop(currentOrder, currentOrder - 1)` (an up-move) succeeds THEN
...
- [ ] 23.2 Implement: update `moveStop()`'s success branch to log `StopReordered(direction,
  currentStopCount())`, deriving `direction` from whether `toOrder < fromOrder`

## 24. TripDetailViewModel — Stop Reordered Failure (BDD)

- [ ] 24.1 Write test: GIVEN `moveStopUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 24.2 Implement: update `moveStop()`'s failure branch for dual-channel emission

## 25. TripDetailViewModel — Stop Departed (BDD)

- [ ] 25.1 Write test: GIVEN `markStopDepartedUseCase` succeeds and returns a `Stop` with `order = 1`
  WHEN `markStopDeparted(stopId)` completes THEN `StopDeparted(stopPosition = 1, stopCount)` is
  logged, in `TripDetailViewModelTest.kt`
- [ ] 25.2 Implement: update `markStopDeparted()`'s success branch to source `stopPosition` from the
...
- [ ] 26.1 Write test: GIVEN `markStopDepartedUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 26.2 Implement: update `markStopDeparted()`'s failure branch for dual-channel emission (already
  logs `OperationFailed`; add the missing `trackException` call)

## 27. TripDetailViewModel — Stop Departure Undone (BDD)

- [ ] 27.1 Write test: GIVEN `undoMarkStopDepartedUseCase` succeeds and returns a `Stop` with `order =
...
- [ ] 27.2 Implement: update `undoMarkStopDeparted()`'s success branch (currently absent) to log
...
- [ ] 28.1 Write test: GIVEN `undoMarkStopDepartedUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 28.2 Implement: update `undoMarkStopDeparted()`'s failure branch (currently absent) for
  dual-channel emission using `AnalyticsOperation.UNDO_MARK_DEPARTED`

## 29. TripDetailViewModel — Place Search Performed (BDD)

- [ ] 29.1 Write test: GIVEN `searchPlacesUseCase` returns 5 suggestions WHEN the search success
  branch runs THEN `PlaceSearchPerformed(suggestionCount = 5)` is logged — this path currently tracks
  nothing, in `TripDetailViewModelTest.kt`
- [ ] 29.2 Implement: update the `OnSearchQueryChanged` success branch (line ~278) to log
  `PlaceSearchPerformed(suggestions.size)`

## 30. TripDetailViewModel — Place Search Failure (BDD)

- [ ] 30.1 Write test: GIVEN `searchPlacesUseCase` fails WHEN the search failure branch runs THEN
...
- [ ] 30.2 Implement: update the `OnSearchQueryChanged` failure branch (line ~287) for dual-channel
  emission using the new `AnalyticsOperation.SEARCH_PLACE`

## 31. TripDetailViewModel — Place Suggestion Selected (BDD)

- [ ] 31.1 Write test: GIVEN a suggestion at index 0 is selected WHEN `OnSuggestionSelected` is
...
- [ ] 31.2 Implement: update `OnSuggestionSelected` handling (line ~302) to log
...
- [ ] 32.1 Write test: GIVEN `getPlaceDetailUseCase` fails WHEN the resolution failure branch runs
  ...
  `TripDetailViewModelTest.kt`
- [ ] 32.2 Implement: update the suggestion-resolution failure branch (line ~323) for dual-channel
  emission using the new `AnalyticsOperation.RESOLVE_PLACE`

## 33. TripDetailViewModel — Route Calculated (BDD)

- [ ] 33.1 Write test: GIVEN `calculateRouteUseCase` succeeds and returns legs summing to 120,000
...
- [ ] 33.2 Implement: update `calculateRoute()`'s success branch to band the returned `List<Leg>`'s
...
- [ ] 34.1 Write test: GIVEN `calculateRouteUseCase` fails WHEN the failure branch runs THEN
...
- [ ] 34.2 Implement: update `calculateRoute()`'s failure branch for dual-channel emission (already
  logs `OperationFailed`; add the missing `trackException` call)

## 35. TripDetailViewModel — Navigation Launched (BDD)

- [ ] 35.1 Write test: GIVEN `navigateToStop(stopId)` is called for the destination at position 2
  ...
  `TripDetailViewModelTest.kt`
- [ ] 35.2 Implement: update `navigateToStop()` (line ~491) to log `NavigationLaunched(stopType,
...
- [ ] 36.1 Inject `AnalyticsClient` into `VeniceApplication` via Hilt member injection
  (`@Inject lateinit var`)
- [ ] 36.2 In `onCreate()`, immediately after `super.onCreate()` and before
...
- [ ] 37.1 Run `./gradlew detektDebug` and resolve any findings
- [ ] 37.2 Run `./gradlew testDebugUnitTest` and confirm all tests pass, including every test added
  in sections 2–35
- [ ] 37.3 Run `./gradlew koverVerify` (or the project's equivalent coverage task) and confirm the
  95%+ bound is maintained
- [ ] 37.4 Cross-check every `AnalyticsEvent`/`AnalyticsOperation`/`AnalyticsErrorType`/
...
- [ ] 37.5 On-device: exercise trip creation, stop add/edit/remove/reorder, mark-departed/undo,
  ...
  in any payload
- [ ] 37.6 On-device: confirm `distance_unit` appears in Logcat before any other analytics log line
