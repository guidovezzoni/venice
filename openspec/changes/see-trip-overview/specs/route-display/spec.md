...
- `trip_detail_recalculation_prompt_message`: "Route not calculated" — plain-language explanation shown in `RouteRecalculationPrompt` when legs/totals are unavailable for a trip with 2 or more stops
- `trip_detail_recalculation_prompt_action`: "Tap to calculate" — tappable call-to-action label shown alongside the prompt message

#### Scenario: String resources defined
- **WHEN** the app is built
- **THEN** all route calculation string resources, including `trip_detail_leg_distance_miles`, `trip_detail_total_distance_label`, `trip_detail_total_distance_unavailable`, `trip_detail_total_duration_label`, `trip_detail_total_duration_unavailable`, `trip_detail_totals_unavailable`, `trip_detail_recalculation_prompt_message`, and `trip_detail_recalculation_prompt_action`, resolve without errors in all supported locales (English, Italian, Spanish)
