...
# Expected: the command detects dirty state and warns about uncommitted changes.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/eval_helpers.sh"

SCENARIO_NAME="exp_vibe_a_story_dirty_state"
...
PROMPT="$(cat "$COMMANDS_DIR/sdlc_exp_vibe_a_story.md")"
