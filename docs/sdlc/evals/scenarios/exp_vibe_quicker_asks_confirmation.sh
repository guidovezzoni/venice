#!/usr/bin/env bash
# Scenario: sdlc_exp_vibe_a_story_quicker asks for user confirmation before proceeding.
# Fixture: git repo on main with a story in New state.
# Expected: output shows warning about fastest option and asks for
...
SCENARIO_NAME="exp_vibe_quicker_asks_confirmation"
...
run_eval_with_args "$FIXTURE_DIR" "$COMMANDS_DIR/sdlc_exp_vibe_a_story_quicker.md" "$OUTPUT_FILE" "99.1.1 test feature" "5" || true

# --- Assert ---
begin_assertions

# The command should present the warning about being the fastest option
assert_output_contains "fastest|quick|two phase|minimal|no.*OpenSpec|no.*DoD" \
    "Output warns about fastest/minimal option"
...
assert_output_not_contains "Phase 1.*complete|Think.*complete|Think.*DONE" \
