#!/usr/bin/env bash
# Scenario: sdlc_exp_vibe_a_story_quicker detects uncommitted changes.
# Fixture: git repo on main with a modified tracked file (dirty working tree).
# Expected: after user confirms the experimental warning, the command detects
#           dirty state and warns about uncommitted changes.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/eval_helpers.sh"

SCENARIO_NAME="exp_vibe_quicker_dirty_state"
...
# --- Run ---
# Prepend "yes" to the prompt so the confirmation gate is passed,
# then the command should hit git safety checks and detect dirty state.
OUTPUT_FILE="$FIXTURE_DIR/eval_output.txt"
echo -e "  ${DIM}Running $EVAL_TOOL for $SCENARIO_NAME...${RESET}"

PROMPT="The user has already confirmed 'yes' to proceed with the experimental command.

$(cat "$COMMANDS_DIR/sdlc_exp_vibe_a_story_quicker.md")"
