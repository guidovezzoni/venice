...
- Epic 9 / Feature 9.2: Firebase Analytics provider — 9.2.1 (FirebaseAnalyticsProvider implementing AnalyticsProvider, constructor-injected FirebaseAnalytics via @Provides, Bundle mapping with Boolean→String conversion for is_first_trip, platform-limit validation with debug/release-mode error handling, ScreenViewed→SCREEN_VIEW special-casing, DI wiring via @Binds @IntoSet, manifest autocapture disabling, CI/CD secret decoding, tracking plan documentation), 9.2.2 (CrashlyticsAnalyticsProvider implementing AnalyticsProvider, constructor-injected FirebaseCrashlytics via @Provides @Singleton, trackException sets operation custom key then calls recordException, logEvent forwards only OperationFailed error_type as custom key, setUserProperty maps TripCountBand and DistanceUnit to custom keys, thread-safe by construction with no mutable instance state, DI wiring via @Binds @IntoSet)
